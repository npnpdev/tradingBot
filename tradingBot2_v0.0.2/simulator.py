import time
from config import FILENAME, STOP_LOSS_PERCENTAGE, TAKE_PROFIT_PERCENTAGE, SLEEP_TIME_BETWEEN_TRADES, update_wallet, save_trade_result, save_trade_log, get_trade_number
    ...
    while position:
        ...
        if position['type'] == 'long':
            if current_price <= stop_loss_price:
                ...
                save_trade_result((0, 0, 1))  # Zaktualizuj wynik o jedną przegraną
                save_trade_log(
                    trade_number=get_trade_number("close"),
                    position_type="close",
                    open_time=latest_row['timestamp'],
                    duration=None,
                    sma={'value': latest_row['SMA_short'], 'signal': 'buy' if latest_row['SMA_short'] > latest_row['SMA_long'] else 'sell'},
                    macd={'value': latest_row['MACD'], 'signal': 'buy' if latest_row['MACD'] > latest_row['MACD_signal'] else 'sell'},
                    rsi={'value': latest_row['RSI'], 'signal': 'buy' if latest_row['RSI'] < 30 else 'sell' if latest_row['RSI'] > 70 else 'neutral'}
                )
            ...
            if current_price >= take_profit_price:
                ...
                save_trade_result((1, 0, 0))  # Zaktualizuj wynik o jedną wygraną
                save_trade_log(
                    trade_number=get_trade_number("close"),
                    position_type="close",
                    open_time=latest_row['timestamp'],  # Tutaj używamy obiektu DataFrame
                    duration=None,
                    sma={'value': latest_row['SMA_short'], 'signal': 'buy' if latest_row['SMA_short'] > latest_row['SMA_long'] else 'sell'},
                    macd={'value': latest_row['MACD'], 'signal': 'buy' if latest_row['MACD'] > latest_row['MACD_signal'] else 'sell'},
                    rsi={'value': latest_row['RSI'], 'signal': 'buy' if latest_row['RSI'] < 30 else 'sell' if latest_row['RSI'] > 70 else 'neutral'}
